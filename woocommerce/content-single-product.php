...
function fw_single_container($atts = [], $content = null){
    return '<div class="container d-flex row px-0 mx-0">'.do_shortcode(stripslashes(htmlspecialchars_decode($content))).'</div>';
}
add_shortcode('fw_single_title', 'fw_single_title');
function fw_single_title(){
    global $product;
    return '<h1 class="product_title entry-title">'.$product->post->post_title.'</h1>';
}
...
function fw_single_gallery(){
    ...
    foreach ($fotos as $ids) {
        ...
        $devolver.= '<a href='.$url.' data-fancybox="gallery" class="d-flex align-items-center" style="background-color: white; position: absolute; top: 0px; left: 0px; opacity: 1;">
