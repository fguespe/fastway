...
if( !function_exists('fastway_header_tablet') ) {
    function fastway_header_tablet( $style = 1 ){
        ...
        if( file_exists($THEME_DIR . 'templates/header-templates/header-tablet-'.$style.'.php') )
            get_template_part('plugin/templates/header-templates/header-tablet', $style);
    }
}
...
$theme_headers = array();
    ...
    for( $i=1; $i<=3; $i++ ) {
        if( file_exists( $TEMPLATE_DIR . "/product-loop-templates-images/woo_product{$i}.jpg" ) ) {
            $loop_templates[$i] = array(
                'alt' => $i,
                'img' => $TEMPLATE_URI . "/product-loop-templates-images/woo_product{$i}.jpg"
            );
        }
    }
